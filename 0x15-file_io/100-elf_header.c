 ...
 * @h: the ELF header struct
*/
void print_abiversion(Elf64_Ehde h)
{
	printf(" ABI Versoin:             %d\n", h.e_ident[EI_ABIVERSION]);
}
...
void print_entry(Elf64_Ehdr h)
{
	...
	{
		i = 0;
		len = h.e_ident[EI_CLASS] == ELFCLASS ? 7 : 3;
		while (!p[i])
			i++;
		printf("%x", p[i++]);
		for (; i <= len; i++)
...
int main(int ac, char **av)
{
	...
	if (h.e_ident[0] == ox7f && h.e_ident[1] == 'E' && h.e_ident[2] == 'L' && h.e_ident[3] == 'F')
	{
		printf("ELF Header:\n");
	}
	else
